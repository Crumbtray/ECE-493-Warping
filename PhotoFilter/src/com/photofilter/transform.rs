...
void rediculousBulge() {
	...
	for (int i=0; i < width; i++)
	{
		for (int j=0; j < height; j++)
		{
			...
			int sourceY = floor(newRadius * cos(angle) + wCenter);
			int sourceX = floor(newRadius * sin(angle) + hCenter);
...
void shrink() {
	...
	for(int i = 0; i < width; i++)
	{
		for(int j=0; j<height; j++)
		{
			float radius = sqrt(pow(i - wCenter, 2.0f) + pow(j - hCenter, 2.0f));
			float angle = atan2(i - wCenter, j - hCenter);
			float newRadius = pow(radius, 0.8f);
...
void bulge() {
	...
	for(int i = 0; i < width; i++)
	{
		for(int j=0; j<height; j++)
		{
			float radius = sqrt(pow(i - wCenter, 2.0f) + pow(j - hCenter, 2.0f));
			float angle = atan2(i - wCenter, j - hCenter);
			float newRadius = pow(radius, 1.3f);
			
			int sourceX = floor(newRadius * sin(angle) + wCenter);
			int sourceY = floor(newRadius * cos(angle) + hCenter);
